...
A **Golden Boot** page (`goldenboot.py`) tracks the race for the tournament's top scorer, built from the same keyless openfootball goals feed (`goals1`/`goals2` per match, persisted to a `scorers` table by the seeder and the 15-minute updater). Real Golden Boot rules apply: **penalties count, own goals don't**, and everyone within two goals of the lead is flagged **in contention**. Each contender also gets a **projected range of additional goals**: their goals-per-match rate — regressed toward a modest prior so a hot opener doesn't extrapolate to a record-shattering tally — carried over the number of matches their team is *expected* to still play, derived from the Stage 8 Monte-Carlo round-reach odds. Since you can't score a fraction of a goal, this is surfaced as a **whole-goal range** (the 80% Poisson interval about that rate, e.g. *+2–7*) rather than a single decimal. So a striker on a team projected deep into the knockouts is credited more upside than one likely to bow out early. A client-side toggle re-ranks the board by current goals or projected finish (works without JS).

### Stage 11 — Future match-ups on the daily map
When you slide the daily map onto a **knockout day whose fixtures aren't set yet**, a slot used to read as a bare placeholder (`W74`, `1E`) with a "?" flag. It now previews the **teams that may play there**: `predict.slot_candidates` reuses the Stage 8 Monte-Carlo aggregate to tally how often each team lands in every knockout slot, and `/api/matches` attaches the top few candidates (team + flag + odds) to each *undecided* side of a fixture (decided sides and the group stage are untouched). The pin shows the favourite's flag dimmed with a "?" corner; the side panel and popup spell out the "may play" cluster — e.g. *Winner Grp E → 🇩🇪 Germany 42% or 🇭🇷 Croatia 18%*. It leans on the projected-bracket cache, so it costs a counter lookup per slot once the sims are warm.
