...
These tests pin down:
  ...
  * the /api/scenarios contract (tree out, validation, budget/limit envelopes);
...
Every LLM call is stubbed at pundits._claude_cli, so the suite is offline,
...
def stub_llm(monkeypatch):
    ...
    Monte-Carlo run, CLI 'installed'. Yields a call-recording list."""
    calls = []
...
# ── the API contract ─────────────────────────────────────────────────────────
def test_ask_returns_scenario_map(client, stub_llm):
...
def test_repeat_question_is_cached_and_free(client, stub_llm):
    client.post('/api/scenarios', json={'question': QUESTION})
    # same question modulo case/whitespace → same cache row, no second LLM call
...
def test_different_question_misses_cache(client, stub_llm):
    client.post('/api/scenarios', json={'question': QUESTION})
    client.post('/api/scenarios', json={'question': 'What if Spain win every match 5-0?'})
    assert len(stub_llm) == 2


def test_question_validation(client, stub_llm):
    for bad in ('', 'hi', 'x' * 301):
        r = client.post('/api/scenarios', json={'question': bad})
...
def test_cli_failure_is_reported_not_cached(client, stub_llm, monkeypatch):
